...
local server = assert(host:connect(("%s:%d"):format("darkplacerebirth.org", "25574")))
...
function Lib:postInit()
    ...
    self.name = Game.save_name --self.getConfig("username") or Game.save_name
...
function Lib:reconnect()
    server = assert(host:connect(("%s:%d"):format("darkplacerebirth.org", "25574")))
end
...
function Lib:updateWorld(...)
    ...
    if currentTime - lastUpdateTime >= THROTTLE_INTERVAL then
        ...
        if Game.world.player.state == "FEATHERFALL" and Game.world.player.platform_state then
            sprite_val =  Game.world.player.platform_state.current_animation --"" ..Game.world.player.sprite.sprite.. "_" ..Game.world.player.sprite.frame.. ""
        ...
        if type(sprite_val) == "table" then
            updateMessage.sprite = nil
        end

        if player.sprite.cust then --send custom sprite data
            updateMessage.cust = player.sprite.cust
        end
...
function Lib:parseServerData(data)
    if data then
                ...
                if playerData.uuid ~= self.uuid then
                    ...
                    if other_player then
                        ...
                        if playerData.cust then --Sync custom data
                            other_player.sprite.cust = playerData.cust
                        end
                    else
                        local otherplr
        ...
        elseif data.command == "teleport" then
            local findstart, findend, mod, map = string.find(data.map or "", "([%w_]+):(.*)")
            if map and map ~= Game.world.map.id then
                if mod == Mod.info.id then
                    Game.world:mapTransition(map, data.x or data.marker, data.y)
                elseif Game.swapIntoMod then
                    -- todo I forgor the args
                else
                    self.chat_box:push("Can't teleport across mods. Attempted to teleport into "..mod..".")
                end
            else
                local x, y = data.x, data.y
                if data.marker then
                    x, y = Game.world.map:getMarker(data.marker)
                end
